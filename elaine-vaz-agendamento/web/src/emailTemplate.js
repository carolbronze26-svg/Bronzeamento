...
export function buildPackageConfirmationEmailHtml({ nome, email, telefone, sessoes }) {
  const linhas = sessoes
    .map((s, i) => `<tr><td style="padding:6px 0; color:#8A8272;">Sessão ${i + 1}</td><td style="padding:6px 0; text-align:right; font-weight:bold;">${s.dataLabel} às ${s.horario}</td></tr>`)
    .join("");
  return `
  <div style="font-family: Arial, sans-serif; background:#0B0A09; padding:24px; color:#F3ECDD;">
    <div style="max-width:420px; margin:0 auto; background:#151109; border:1px solid #2A241A; border-radius:16px; overflow:hidden;">
      ...
      <div style="padding:22px;">
        <h2 style="margin:0 0 6px; color:#F3ECDD; font-size:20px;">Pacote de 4 sessões confirmado</h2>
        <p style="margin:0 0 18px; color:#9C9384; font-size:13px;">Beleza que você vê, saúde que você sente.</p>
        <table style="width:100%; border-collapse:collapse; font-size:14px;">
          <tr><td style="padding:6px 0; color:#8A8272;">Cliente</td><td style="padding:6px 0; text-align:right; font-weight:bold;">${nome || "-"}</td></tr>
          ${linhas}
...
export function buildConfirmationEmailHtml({ nome, servico, data, horario, telefone, email, amigaNome, amigaTelefone }) {
  const linhaAmiga = amigaNome
    ? `
      <tr><td colspan="2" style="padding:14px 0 4px; color:#E8CE85; font-size:13px; font-weight:bold; border-top:1px solid #2A241A;">Promoção Amiga chama Amiga</td></tr>
      <tr><td style="padding:6px 0; color:#8A8272;">Nome da amiga</td><td style="padding:6px 0; text-align:right; font-weight:bold;">${amigaNome}</td></tr>
      <tr><td style="padding:6px 0; color:#8A8272;">WhatsApp da amiga</td><td style="padding:6px 0; text-align:right; font-weight:bold;">${amigaTelefone || "-"}</td></tr>
    `
    : "";
