...
function buildLinhaAmiga(amigaNome, amigaTelefone) {
  if (!amigaNome) return "";
  return `
...
export function buildConfirmationEmailHtml({
  nome, servico, data, horario, telefone, email, amigaNome, amigaTelefone,
}) {
  const linhaAmiga = buildLinhaAmiga(amigaNome, amigaTelefone);
...
export function buildPackageConfirmationEmailHtml({
  nome, email, telefone, sessoes, amigaNome, amigaTelefone,
}) {
  const linhas = sessoes
    .map(
      (s, i) =>
        `<tr><td style="padding:6px 0; color:#8A8272;">Sessão ${i + 1}</td><td style="padding:6px 0; text-align:right; font-weight:bold;">${s.dateLabel} às ${s.horario}</td></tr>`
    )
    .join("");
  const linhaAmiga = buildLinhaAmiga(amigaNome, amigaTelefone);
