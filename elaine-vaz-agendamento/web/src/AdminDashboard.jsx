...
export default function AdminDashboard() {
  ...
  // Selos de fidelidade de cada cliente (coleção "usuarios", indexada por uid)
  useEffect(() => {
    if (!isAdmin) return;
  ...
  async function setStatus(item, novoStatus) {
    try {
      ...
      await sincronizarSeloFidelidade(item, novoStatus);
    } catch (err) {
      alert("Não foi possível atualizar o status. Tente novamente.");
    }
  }

  // Selo de fidelidade: soma 1 quando o atendimento é marcado como
  // concluído ("confirmado"), e desfaz se você reverter o status por
  // engano. Usa o campo `seloContado` no próprio agendamento pra nunca
  // contar o mesmo atendimento duas vezes.
  //
  // Ao chegar a 10 selos, zera o contador e soma 1 em premiosDisponiveis
  // (sessão grátis). OBS: se você desfizer um status DEPOIS que ele já
  // rendeu um prêmio, o prêmio já concedido não é removido automaticamente.
  async function sincronizarSeloFidelidade(item, novoStatus) {
    if (!item.usuarioId) return; // agendamento sem cliente vinculado (não deveria acontecer)

    const vaiConcluir = novoStatus === "confirmado" && !item.seloContado;
    const vaiDesfazer = novoStatus !== "confirmado" && item.status === "confirmado" && item.seloContado;
    if (!vaiConcluir && !vaiDesfazer) return;

    const userRef = doc(db, "usuarios", item.usuarioId);
    const agendamentoRef = doc(db, "agendamentos", item.id);

    try {
      await runTransaction(db, async (transaction) => {
        const userSnap = await transaction.get(userRef);
        const atual = userSnap.exists() ? userSnap.data() : {};
        let selos = atual.selosFidelidade || 0;
        let premios = atual.premiosDisponiveis || 0;
        ...
        if (vaiConcluir) {
          ...
          transaction.set(userRef, { selosFidelidade: selos, premiosDisponiveis: premios }, { merge: true });
          transaction.update(agendamentoRef, { seloContado: true });
        } else if (vaiDesfazer) {
          selos = Math.max(0, selos - 1);
          transaction.set(userRef, { selosFidelidade: selos }, { merge: true });
          transaction.update(agendamentoRef, { seloContado: false });
        }
      });
    } catch (err) {
      console.error("Não foi possível atualizar o selo de fidelidade:", err);
    }
  }
        ...
        {activeTab === "andamento" && (
          <>
            <AdminSection
              ...
              contagemPorCliente={contagemPorCliente} avaliacoes={avaliacoes} usuarios={usuarios} onSetStatus={setStatus} onDelete={excluirAgendamento}
            />
            <AdminSection
...
function AdminSection({ title, icon, items, contagemPorCliente, avaliacoes, usuarios, onSetStatus, onDelete }) {
  return (
    <div className="adminSection">
      ...
      {items.map((item) => {
        ...
        const loyaltyInfo = usuarios?.[item.usuarioId];
          ...
          <div key={item.id} className="adminCard">
            <div className="adminCardMain">
              <div className="adminCardName">
                ...
                {loyaltyInfo && (
                  <span className="adminLoyaltyTag" title="Selos de fidelidade">
                    🏅 {loyaltyInfo.selosFidelidade || 0}/10
                    {loyaltyInfo.premiosDisponiveis > 0 && ` · 🎁 ${loyaltyInfo.premiosDisponiveis}`}
                  </span>
                )}
              </div>
